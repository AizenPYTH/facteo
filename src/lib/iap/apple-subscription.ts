...
/**
 * Feuille native Apple Offer Codes (StoreKit). Appareil réel uniquement.
 * Le code est saisi chez Apple — INVEQ ne l’intercepte jamais et ne débloque rien sans IAP.
 */
export async function redeemAppleOfferCode(): Promise<ApplePlanConfirmResult | null> {
  if (Platform.OS !== 'ios') {
    throw new Error('Les codes promo Apple sont disponibles sur iPhone.');
  }

  await initConnection();

  return await new Promise<ApplePlanConfirmResult | null>((resolve, reject) => {
    let settled = false;
    let sheetClosed = false;

    const finish = (error?: unknown, result?: ApplePlanConfirmResult | null) => {
      if (settled) {
        return;
      }
      settled = true;
    ...
    const errored = purchaseErrorListener((error) => {
      ...
      finish(error instanceof Error ? error : new Error(message || 'Code promo Apple impossible.'));
    });

    void openRedeemOfferCode()
      .then((purchase) => {
        if (purchase && isKnownPaidPurchase(purchase)) {
          void finalizePurchase(purchase)
            .then((result) => finish(undefined, result))
            .catch((error) => finish(error));
          return;
