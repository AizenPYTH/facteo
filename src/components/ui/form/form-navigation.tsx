...
import type { ReturnKeyTypeOptions, SubmitBehavior, TextInput } from 'react-native';
...
export function useFieldNavigation(options?: { multiline?: boolean; enabled?: boolean }): FieldNavigationProps | null {
  ...
  return {
    ...
    // `blurOnSubmit` est déprécié depuis React Native 0.86 : `submitBehavior`
    // le remplace. Sur un champ multiligne, Entrée insère un retour à la ligne —
    // fermer le clavier rendrait toute description de plusieurs lignes
    // impossible à saisir. Ces champs sont d'ailleurs déjà exclus de la chaîne
    // (voir `chain`). Sur un champ intermédiaire, on soumet sans fermer le
    // clavier, puisque le focus part au champ suivant.
