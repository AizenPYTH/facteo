 ...
 *
 * Aucun secret eBay ne transite ici : le site ne connaît ni le client_id, ni
 * le client_secret, ni le RuName, ni aucun jeton. Il demande une URL
 * d'autorisation à l'Edge Function et lit une vue qui n'expose aucun secret.
 */
...
export function startEbayOAuth(
  companyId: string,
  options: { environment?: IntegrationEnvironment; redirectTo?: string } = {},
) {
  return callEdgeFunction<{
    ...
    'ebay-oauth-start',
    { companyId, environment: options.environment, redirectTo: options.redirectTo },
...
export async function disconnectEbay(companyId: string): Promise<void> {
  ...
  if (error) throw new Error('Déconnexion eBay impossible.');
}
...
export async function fetchEbayIntegration(companyId: string): Promise<IntegrationSummary | null> {
  ...
  if (error) throw new Error('Impossible de lire l’état de la connexion eBay.');
...
export async function fetchEbayOrders(
  ...
  if (error) throw new Error('Impossible de charger les commandes eBay.');
...
export async function fetchEbayOrderById(
  ...
  if (error) throw new Error('Impossible de charger la commande eBay.');
