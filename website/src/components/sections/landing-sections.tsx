    ...
    <section className="gradient-hero relative overflow-hidden px-6 pb-24 pt-20 lg:px-8 lg:pt-28">
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <FadeIn delay={0.2}>
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-muted">{HERO.subtitle}</p>
          </FadeIn>
          <FadeIn className="mt-10 flex flex-wrap items-center justify-center gap-4" delay={0.3}>
        ...
        <FadeIn className="mt-14 grid gap-4 sm:grid-cols-3" delay={0.35}>
          {[
            { label: 'Utilisateurs actifs', value: 2400, suffix: '+' },
            { label: 'Documents générés', value: 180000, suffix: '+' },
            { label: 'Temps gagné / mois', value: 12, suffix: ' h' },
          ].map((stat) => (
            <div
              className="rounded-2xl border border-border/80 bg-surface/80 p-5 text-center shadow-sm backdrop-blur-sm"
              key={stat.label}>
        ...
        <FadeIn className="mt-16" delay={0.45}>
          <motion.div
            animate={{ y: [0, -8, 0] }}
            className="mx-auto max-w-4xl overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl shadow-slate-200/60"
            ...
            <div className="grid grid-cols-3 gap-4 p-6">
              {['CA mensuel', 'Factures impayées', 'Clients actifs'].map((label, i) => (
                <div className="rounded-xl border border-border bg-slate-50 p-4 text-left" key={label}>
        ...
        <StaggerItem key={plan.id}>
          <div
            ...
            <ul className="mt-6 flex-1 space-y-3">
              {plan.features.map((f) => (
                <li className="flex items-center gap-2 text-sm text-foreground" key={f}>
                  <span className="text-primary">✓</span> {f}
                </li>
              ))}
            </ul>
            <div className="mt-8">
              <Button href={APP_REGISTER_URL} variant={plan.highlighted ? 'primary' : 'secondary'}>
                {plan.cta}
              </Button>
            </div>
          </div>
