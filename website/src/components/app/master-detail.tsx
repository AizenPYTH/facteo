...
export function MasterDetailLayout({
  ...
  return (
    <div className={cn('flex h-full min-h-0 w-full', className)}>
      <div className="flex w-[340px] shrink-0 flex-col border-r border-slate-200 bg-white xl:w-[380px]">
        {list}
      </div>
      <div className="flex min-w-0 flex-1 flex-col bg-slate-50">{detail}</div>
      {sidebar ? (
        <>
          {resizable ? <ResizeHandle onResize={onSidebarResize!} /> : null}
          <div
            className={cn(
              'hidden min-h-0 shrink-0 flex-col border-l border-slate-200 bg-white xl:flex',
