...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      return jsonResponse({ error: 'Configuration serveur incomplete.' }, 500);
    }

    if (!openAiApiKey) {
      return jsonResponse({ error: "L'assistant IA n'est pas encore configure." }, 503);
    }
    ...
    const modelOutput = await requestOpenAiJsonSchema({
      ...
      systemPrompt: [
        ...
        'Si aucune TVA n est visible mais le prix semble TTC, laisse vat a null.',
        'Si aucune information n est visible, retourne des champs vides/null sans inventer.',
        'La quantite par defaut est 1. L unite par defaut est "piece".',
        'La devise par defaut est EUR pour un contexte francophone.',
        'confidence doit representer ton niveau de certitude entre 0 et 1.',
        'Si plusieurs produits sont visibles (tableau, liste, feuille Excel, capture e-commerce), renseigne products avec tous les produits detectes.',
        'Chaque entree de products doit contenir les memes champs que le produit principal.',
        'Le produit principal (champs top-level) doit etre le premier element pertinent detecte.',
      ].join('\n'),
      userContent: [
        {
          type: 'input_text',
          text: 'Analyse cette image et retourne uniquement les donnees produit au format JSON.',
        },
        {
          type: 'input_image',
    ...
    if (message === 'OPENAI_TIMEOUT') {
      return jsonResponse({ error: "L'assistant IA met trop de temps a repondre. Reessayez." }, 504);
    }
